...
{
  #ifdef FTP_DEBUG
	Serial.println("Sending client connected welcome message");
  #endif
...
boolean FtpServer::processCommand()
{
  ...
  if( ! strcmp( command, "CDUP" ) || ( ! strcmp( command, "CWD" ) && ! strcmp( parameters, ".." )))
  {
   ...
   // client << F("250 Ok. Current directory is ") << cwdName << eol;
    ...
    client.println("250 Ok. Current directory is ");
  }
  //
  //  CWD - Change Working Directory
  //
  else if( ! strcmp( command, "CWD" ))
    { /* There was a bug here, you can only give one response to
	 client. Fixed, Leon */
...
#ifdef FTP_DEBUG
    if (data.connected()) {
      Serial.println("PASV : DataServer is connected.");
    }
  ...
  //
    ...
    {
#ifdef FTP_DEBUG
      Serial.println("Entering PORT command. LEON ");
#endif
    if (data) data.stop();
    ...
    // get port of data client. DONT LEON
    dataPort = 256 * atoi( ++ p );
    p = strchr( p, ',' );
    dataPort += atoi( ++ p );
    if( p == NULL )
...
#ifdef FTP_DEBUG
	Serial.println("MSLD: No data connection");
#endif
      }
    else
      {
#ifdef FTP_DEBUG
	Serial.println( "MLSD: Accepted data connection");
#endif
	client.println( "150 Accepted data connection");
	uint16_t nm = 0;
	File dir= SD.open(cwdName);
...
#endif
	      int pos = fn.lastIndexOf("/"); /* looking for the
...
#ifdef FTP_DEBUG
  Serial.print("Entering dataConnect. data.connected : ");
  Serial.println(data.connected());
#endif
  if (!data.connected())
  {
    while (!dataServer.hasClient() && millis() - startTime < 1000)
      { //delay(100);
	yield();
      }
    if (dataServer.hasClient()) {
      ...
      data = dataServer.available();
#ifdef FTP_DEBUG
      Serial.println("dataConnect: OK, dataServer has client....");
#endif		
    }
    else {
      Serial.println("dataConnect: dataServer.hasClient is false!");
    }
  }
#ifdef FTP_DEBUG
  Serial.print("In dataConnect. Now data.connected : ");
  Serial.println(data.connected());
#endif
