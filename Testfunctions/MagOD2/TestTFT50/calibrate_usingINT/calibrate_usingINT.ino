/* Test to see if we can use interupt driven touchscreen. Did not work, and did not continue. Leon, April 2019 */
...
void waitForTouchEvent(tsPoint_t * point)
{
  ...
  //Apparently RA8875_INT is always down
  
  /* Wait around for a new touch event (INT pin goes low) */
  /*  while (digitalRead(RA8875_INT)) */
  /* //  while (true) */
  /* { */
  /*   Serial.print(".");Serial.println(digitalRead(RA8875_INT)); */
  /* } */
...
void setup() 
{
  ...
  //attachInterrupt(digitalPinToInterrupt(RA8875_INT), touchEvent, LOW);
  attachInterrupt(RA8875_INT, touchEvent, LOW);
...
void loop() 
{
  ...
  delay(2);
  //Serial.println(digitalRead(RA8875_INT));
