...
void loop()
{
  int adc0, adc1, adc2, adc3; //Bit values of adc
  char string[15]; //Buffer holding string for dtostrf
  int Chan_i = LEDChannelRed;
  ...
  for(int color = colors[0]; color <= size_colors; color = color + 1 )
    { 
      ...
      case RED:
	Serial.println("RED");
	Chan_i = LEDChannelRed;
	break;
      case GREEN:
	Serial.println("GREEN");
	Chan_i = LEDChannelGreen;
	break;
      case BLUE:
	Serial.println("BLUE");
	Chan_i = LEDChannelBlue;
	break;
      default:
	...
	Serial.println(color);
      ...
      for(int intensity = 0; intensity <= pow(2,LEDresolution);
	  intensity = intensity + 5 )
	{
	  ledcWrite(Chan_i,intensity);
	  /* Read ADC1115 ports */
	  adc0 = 0, adc1 = 0, adc2 = 0, adc3 = 0;
	  int N = 10;
	  for (int i = 0; i < N; i++)
	    {
	      adc0 = adc0 + ads0.readADC_SingleEnded(0);
	      adc1 = adc1 + ads0.readADC_SingleEnded(1);
	      adc2 = adc2 + ads0.readADC_SingleEnded(2);
	      adc3 = adc3 + ads0.readADC_SingleEnded(3);
	    }
	  Serial.print(intensity);
	  Serial.print(", ");
	  // Serial.print(adc0/N);
	  // Serial.print(", ");
	  Serial.print(adc1/N); //photoDiode
	  Serial.print(", ");
	  // Serial.print(adc2/N);
	  // Serial.print(", ");
	  Serial.println(adc3/N); // refDiode
	  /* Write to TFT screen */
	  ...
	  tft.textWrite(dtostrf(adc0/N,5,0,string),6);
	  
	  tft.textSetCursor(1, 15);
	  tft.textWrite("ADC1: ");
	  tft.textSetCursor(40, 15);
	  tft.textWrite(dtostrf(adc1/N,5,0,string),6);

	  tft.textSetCursor(1, 30);
	  tft.textWrite("ADC2: ");
	  tft.textSetCursor(40, 30);
	  tft.textWrite(dtostrf(adc2/N,5,0,string),6);
	  
	  tft.textSetCursor(1, 45);
	  tft.textWrite("ADC3: ");
	  tft.textSetCursor(40, 45);
	  tft.textWrite(dtostrf(adc3/N,5,0,string),6);
	  
	  tft.textSetCursor(1, 60);
	  tft.textWrite("INTE: ");
	  tft.textSetCursor(40, 60);
	  tft.textWrite(dtostrf(intensity,5,0,string),6);
	}
