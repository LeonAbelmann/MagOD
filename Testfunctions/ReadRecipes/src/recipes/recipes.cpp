...
was found */
bool readOneLine(File file, String& line){
  ...
  while (file.available() && not linefound){
      line=""; // clear line contents
    // Read next line.
    // as long as the file has more chars, and we don't run into the end of line, do
    while (file.available() && !endofline) { 
      ch = file.read(); // read one single character into ch
      if (ch != '\n') { // if it is not \n, than add it to line
	line += ch;
	//Serial.print(ch);
      }
...
bool readSequence(File recipeFile, sequence& seq){
  ...
  /* Read until you find EndSequence. The order of checking is
     ...
     line too many */
  while(not endSequence && readOneLine(recipeFile, line)){ 
...
int recipes::LoadRecipes(File recipeFile, recipe recipes_array[]){
  ...
  recipeNumber=-1;
  while(recipeFile.available() && not error){
    // Read one line from the recipe file, ignore comment lines
    if (readOneLine(recipeFile, line)) {
	...
	if (comparestring(col[1],"Recipe")
	  ...
	  while (readOneLine(recipeFile, line) && not recipeEnd && not error){
		...
		if (not readSequence(recipeFile,
				  recipes_array[recipeNumber].recipe_sequence))
...
/* LED_init()
This is very akward. If we have a initialization step in the recipe, we don't have this problem anymore and it can be removed. We can also choose to introduce a delay when we switch the ledcolor in myled::Set_LED_color. LEON
...
*/
void recipes::LED_init(recipe recipes_array[],int recipe_num)
{
  LED_type = recipes_array[recipe_num].recipe_sequence.led[0].color;
#if defined(_MAGOD2)
  LED_intensity[LED_type]=recipes_array[recipe_num].recipe_sequence.led[0].intensity;
#endif
